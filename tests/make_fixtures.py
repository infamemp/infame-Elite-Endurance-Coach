...
def case_taper_active():
    """Runner with a declared A-priority marathon one day out (see
    declared_profile_taper_active below, written alongside this fixture's
    athlete_data.json). Before the taper_check fix, this path was
    unreachable in every fixture -- the function only ever saw Intervals.icu
    event fields it couldn't match, so it always fell into the
    'not applicable' branch, and the branch that actually computes a
    projected-TSB verdict against a target range had zero coverage. The pmc
    ramp here is tuned so the projected TSB at the goal date lands inside
    the 'marathon' target band (10-20): applicable=True,
    verdict=in_target_range.
    """
    return wrap(
        profile("Taper Runner", weight=64.0, ftp=None),
        wellness(120, 62),
        pmc_series(120, 35, 50, -10),
        activities(36, 3, 70, decoupling=[3.5, 4.0, 3.2, 3.8], atype="Run",
                   watts=None),
        pace_curves(
            {"400": 62, "1000": 172, "3000": 570, "5000": 990, "10000": 2100,
...
def declared_profile_taper_active():
    """The companion config/athletes/<id>.yaml for case_taper_active, in the
    minimal form taper_check actually reads (goals[].priority/date/
    event_type/description). Date is relative to today for the same reason
    every other fixture date is: a fixed date would age out of the taper
    window within days."""
    race_date = (date.today() + timedelta(days=1)).isoformat()
    return (
        "# Auto-generated by tests/make_fixtures.py -- do not edit by hand.\n"
        "# Minimal declared profile: only the goals[] fields taper_check reads.\n"
        "id: _test_taper_active\n"
        "goals:\n"
...
CASES = {
    ...
    "taper_active": case_taper_active,
}

# Fixtures that also need a declared profile (config/athletes/<id>.yaml)
# copied into place at test time -- see build_for_fixture() in run_tests.py.
DECLARED_PROFILES = {
    "taper_active": declared_profile_taper_active,
}
...
def main():
    ...
    for name, fn in CASES.items():
        ...
        if name in DECLARED_PROFILES:
            with open(os.path.join(dest, "declared_profile.yaml"), "w",
                      encoding="utf-8") as f:
                f.write(DECLARED_PROFILES[name]())
